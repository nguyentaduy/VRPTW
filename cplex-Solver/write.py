...
def solve(file_name): # populate by rows
    ...
    try:
        ...
        for i in range(N + 1):
            for j in range(N+1):
                for k in range(K):
                    if sol_[get_index_x(K, N, k, i, j)] > 0.5:
                        arcs.append([x[i], y[i], x[j], y[j]])
        # plt.show()
        return sol, arcs
    except CplexError as exc:
        print(exc)
        return 0
def write_result(file_name, arcs_file, sol, arcs):
    with open(file_name, "w") as f:
        f.write(str(sol))
    with open(arcs_file, "w") as f:
        for arc in arcs:
            f.write(str(arc[0]) + " " + str(arc[1]) + " " + str(arc[2]) + " " + str(arc[3]) + "\\n")
# In[18]:
if __name__ == '__main__':
    ...
    for file in train_set:
        ...
        if c %s 10 == %s:
            print(file)
            sol, arcs = solve("../Test_in/1/"+file)
            write_result("../cplex-in-out/full_rc101_0/" + file + "_full_unlimit", "../cplex-in-out/full_rc101_0/" + file + "_arcs_unlimit", sol, arcs)

"""
for i in range(0,10):
    f = "cplex-Solver-" + str(i) + ".py"
    with open(f, "w") as f:
        f.write(towrite % ('%', str(i)))
        # f.write(towrite)
